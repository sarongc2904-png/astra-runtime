...
function localAndromeda(){
  ...
  return [{evidence_id:'ANDROMEDA:1',family:'andromeda_local',source_file:'META_ANDROMEDA_VERIFIED_2026.md',excerpt:fs.readFileSync(ANDROMEDA_PATH,'utf8').slice(0,1600),match_type:'local_versioned_source'}];
}
...
async function buildEvidence(){
  ...
  const ids=bounded.map(e=>e.evidence_id);
  if(ids.some(id=>!id||id.length>32)) throw new Error('evidence_id_contract_violation');
  if(new Set(ids).size!==ids.length) throw new Error('duplicate_evidence_ids');
...
function adjudicate(out,evidence){
  ...
  if(Array.isArray(out.sources_used)) {
    for(const s of out.sources_used) if(!allowedSources.has(s)) violations.push(`UNRESOLVED_SOURCE:${s}`);
  } else {
    violations.push('SOURCES_USED_NOT_ARRAY');
  }
  ...
  if(!Array.isArray(out.unsupported_claims)) violations.push('UNSUPPORTED_CLAIMS_NOT_ARRAY');
  else if(out.unsupported_claims.length>0) violations.push('UNSUPPORTED_CLAIMS_PRESENT');

  // Scan only proposed creative content/decisions, not the brief constraints that explicitly say what NOT to invent.
  const claimSurface=JSON.stringify({
    concept:out.concept||{},
    art_direction:out.art_direction||{},
    copy:out.copy||{},
    decisions:Array.isArray(out.decisions)?out.decisions.map(d=>d.decision):[],
  }).toLowerCase();
  const prohibited=[/\b\d+\s*%\b/,/\bgarant(?:iza|izado|izados|izada|izadas|ía|izamos)\b/,/\bresultados? asegurados?\b/,/\bdescuento\s+\d+/,/\btestimonios? de clientes\b/];
  if(prohibited.some(r=>r.test(claimSurface))) violations.push('POTENTIAL_UNSUPPORTED_PERFORMANCE_OR_PROOF_CLAIM');
...
async function main(){
  ...
  console.log(`[ASTRA_NEXT_09] evidence_gate=PASS evidence=${evidence.length}/12 ids=${JSON.stringify(evidence.map(e=>e.evidence_id))} sources=${JSON.stringify([...new Set(evidence.map(sourceOf).filter(Boolean))])}`);
  await waitApi();
  const token=await localAuth();
  const apiKey=await devApiKey(token);
  const prompt=buildPrompt(evidence);
  const body={message:prompt,mode:'chat',sessionId:'astra-next-09-grounded-creative-v2',attachments:[],reset:true};
